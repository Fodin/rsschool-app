...
const getAvailableBadges = ({ roles, coursesRoles }: Session, id: number) => {
  const userRoles = roles[id] ?? [];
  const userCourseRoles = coursesRoles ? coursesRoles[id] : [];
  const isAvailableSpecialBadges = [userRoles, ...(userCourseRoles ?? [])].some(role =>
    rolesForSpecialBadges.includes(role),
  );
